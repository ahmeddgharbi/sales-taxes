'use strict';
const ProductHandler = require('./ProductHandler');
const Product = require('./Product');

function Input(){

  this.products = [];

  this.total_no_tax = 0;
  this.sales_tax = 0;


}

/**
 * Add product to the input as string
 * @param product
 */

Input.prototype.scanProduct = function(product){

  var product_properties = ProductHandler.analize(product);
  var quantity = product_properties.quantity;
  var new_product = new Product(product_properties);

  this.products.push({
      quantity: quantity,
      product: new_product
    });

  // update total of input
  this.updateTotal(new_product, quantity);

};


/**
 * Add product to the input as product
 * @param product
 */

Input.prototype.addProduct = function(product, quantity){

  var new_product = new Product(product);

  this.products.push({
    quantity: quantity || 1,
    product: new_product
  });

  // update total of input
  this.updateTotal(new_product, quantity);

};

/**
 * Return total and sales_tax as object
 * @param product
 * @returns {*}
 */
 Input.prototype.updateTotal = function(product, quantity){

  var price      = product.getPrice();
  var priceTaxed = ProductHandler.getTaxedPriceOf(product);

  this.total_no_tax += (quantity * price);
  this.sales_tax += quantity * (priceTaxed - price);

};


/**
 * Build the output
 * @returns {string}
 */

 Input.prototype.output = function(){

  var output = '';

  // calculate taxes and build output
  this.products.forEach(
    function(toBuy){
      output += toBuy.quantity + ' ' + toBuy.product.toString() + '\n';
    }
  );

  output += 'Sales Taxes: '+ this.getSalesTax() + '\n';
  output += 'Total: ' + this.getTotal();

  return output;

};

/**
 * Get amount of tax
 * @returns {string}
 */
Input.prototype.getSalesTax = function(){
  return this.sales_tax.toFixed(2);
};

/**
 * Get total of Input
 * @returns {string}
 */
 Input.prototype.getTotal = function(){
  return (this.total_no_tax + this.sales_tax).toFixed(2);
}

module.exports = Input;